...
public class HashTable {
     ...
     */
    //PRUEBA
    //ERROR 1 -> SI AÑADIMOS TENEMOS QUE SUMAR AL ITEMS Y NO LO HACE
    //ERROR 2 -> NO SOBREESCRIBE UNA KEY EN UNA POSICIÓN YA UTILIZADA
    public void put(String key, String value) {
        int hash = getHash(key);
        final HashEntry hashEntry = new HashEntry(key, value);
        //Si no existe la entrada en el array la crea
        if(entries[hash] == null) {
            entries[hash] = hashEntry;
        } else {
            //Si ya existe guarda los valores en una variable temporal
            HashEntry temp = entries[hash];

            //Mientras temp.next no exista, temp pasa a ser temp.next
            while(temp.next != null) {
                temp = temp.next;
            }
            //Si la key que está en temp no existe o no es igual a una existente
            if (!temp.key.equals(key)) {
                //Temp.next pasa a ser hashEntry
                temp.next = hashEntry;
            } else {
                //Si no, si existe esa clave, sobreescribe el valor de la misma
                temp.value = value;
            }
            hashEntry.prev = temp;
        }
        //Como hemos añadido una entrada hay que sumar un ITEM
        ITEMS++;
    }
//    ORIGINAL
//    public void put(String key, String value) {
...
//    }
     ...
     */
    //PRUEBA
    //ERROR 4 -> NO RESTA EN EL TAMAÑO DE ITEMS AL DROPEAR UN ITEM
    //ERROR 5 -> CUANDO ELIMINAMOS LA PRIMERA KEY Y TIENE COLISIONES, ELIMINA TODAS LAS SIGUIENTES
    //ERROR 6 -> ELIMINA LA KEY DE ENMEDIO O FINAL SIN ELIMINAR EL RESTO
    public void drop(String key) {
        int hash = getHash(key);
        //Si no existe la entrada en el array la crea
        if(entries[hash] != null) {
            HashEntry temp = entries[hash];
            //Mientras la key de la variable temp no sea igual a una del hash
            while(!temp.key.equals(key)) {
                temp = temp.next;
            }

            //Si no existe un prev. es que es el primer elemento
            if(temp.prev == null) {
                //Si el elemento tiene un next
                if (temp.next != null) {
                    //Convertimos o borramos el anterior de next
                    temp.next.prev = null;
                }
                //El elemento temp.next se convierte en el primero
                entries[hash] = temp.next;
            } else {
                //Aqui movemos la posicion de los valores al eliminar un valor
                if(temp.next != null) temp.next.prev = temp.prev;   //esborrem temp, per tant actualitzem l'anterior al següent
                temp.prev.next = temp.next;                         //esborrem temp, per tant actualitzem el següent de l'anterior
            }
            //Tenemos que restar el item que hemos eliminado
            ITEMS--;
        }
    }
//  ORIGINAL
